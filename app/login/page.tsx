...
function LoginContent() {
  ...
  return (
    <div className="min-h-screen bg-[#FCF9F7] flex flex-col selection:bg-blush/20 relative overflow-hidden" suppressHydrationWarning>
      {/* Subtle Background Radial Depth */}
      <div className="absolute top-0 left-0 w-full h-full pointer-events-none overflow-hidden -z-10">
        <div
          className="absolute top-0 left-0 w-full h-full"
          style={{
            background: `radial-gradient(circle at top left, rgba(255, 185, 170, 0.10), transparent 42%)`
          }}
        />
      </div>
            ...
            <div className="brand-panel flex flex-col items-start justify-center space-y-10 lg:space-y-12">
              <div className="brand-content w-full space-y-8 text-left">
                ...
                <div
                  ...
                  style={{ backgroundColor: 'rgba(255, 170, 150, 0.12)' }}
                >
                  <span className="text-[11px] md:text-[12px] font-black uppercase tracking-[2px] text-[#ef7f6d]">
                ...
                <div className="space-y-6">
                  <h1
                    ...
                    style={{ fontSize: 'clamp(52px, 5.5vw, 84px)', maxWidth: '640px', color: 'rgba(0,0,0,0.82)', letterSpacing: '-0.03em' }}
                  >
                    Premium Essentials <br className="hidden md:block" />
                    for <span style={{ color: '#ef7f6d' }}>Motherhood</span>
                  </h1>
                  <p
                    className="brand-description font-sans font-medium leading-[1.7]"
                    style={{ fontSize: '20px', maxWidth: '520px', color: 'rgba(0,0,0,0.62)' }}
                  >
                ...
                <div className="trust-grid grid grid-cols-3 gap-[18px] w-full max-w-[580px]">
                  {[
                    ...
                    <div key={i} className="trust-card bg-white/72 border border-black/5 rounded-[22px] p-5 shadow-[0_4px_14px_rgba(0,0,0,0.04)] backdrop-blur-[8px] flex flex-col justify-between min-h-[120px] text-left">
                      <div className="w-[22px] h-[22px] text-blush">
                        <item.icon className="w-full h-full" strokeWidth={1.5} />
                      </div>
                      <div className="space-y-1">
                        <p className="text-[10px] font-black uppercase tracking-widest leading-none" style={{ color: 'rgba(0,0,0,0.82)' }}>{item.title}</p>
                        <p className="text-[9px] font-bold leading-relaxed" style={{ color: 'rgba(0,0,0,0.62)' }}>{item.desc}</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="auth-panel relative flex justify-center lg:justify-start">
              ...
              >
                <div className="relative z-10 flex flex-col items-center text-center">
                  {/* Auth Icon */}
                  <div className="auth-icon w-[72px] h-[72px] rounded-full bg-[#ffb4a0]/[0.12] flex items-center justify-center mb-10 shadow-sm">
                    <User className="w-8 h-8 text-[#ef7f6d]" strokeWidth={1} />
                  </div>

                  {/* Auth Heading */}
                  <h2 className="auth-heading text-[38px] md:text-[48px] font-serif font-bold mb-3 tracking-tight leading-[1.1]" style={{ color: 'rgba(0,0,0,0.82)' }}>
                    Welcome <span className="text-[#ef7f6d] italic font-normal">Back</span>
                  </h2>

                  {/* Auth Description */}
                  <p className="auth-description text-[17px] font-sans font-medium mb-12 leading-[1.7] max-w-[280px] mx-auto" style={{ color: 'rgba(0,0,0,0.62)' }}>
                  ...
                  <div className="w-full">
                    {authMethod === "choice" ? (
                      <div className="flex flex-col gap-6">
                        {/* Google Button - 58px Height Primary CTA */}
                        <button
                          onClick={handleGoogleLogin}
                          disabled={loading}
                          className="google-btn w-full h-[58px] rounded-[20px] font-semibold bg-gradient-to-br from-[#f08d7d] to-[#f47d6d] text-white hover:shadow-[0_12px_24px_rgba(240,125,109,0.18)] hover:-translate-y-[1px] transition-all duration-300 active:scale-[0.98] disabled:opacity-80 flex items-center justify-center gap-4 group"
                        >
                          {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : (
                            <>
                              <div className="w-5 h-5 bg-white rounded-full flex items-center justify-center p-1 shadow-sm transition-transform group-hover:scale-110">
                        ...
                        <div className="divider relative py-1 flex items-center justify-center">
                          ...
                          <span className="relative px-4 bg-white text-black/30 font-bold text-[10px] uppercase tracking-[0.2em]">or</span>
                        </div>

                        {/* Phone Button - Softer Active Style */}
                        <button
                          onClick={() => setAuthMethod("phone")}
                          className="otp-btn w-full h-[58px] rounded-[20px] font-semibold bg-white border border-[#ffd8d1] text-charcoal hover:bg-[#fff2ef] hover:border-[#ffc8bc] transition-all duration-300 active:scale-[0.98] flex items-center justify-center gap-3 group"
                        >
                          <Phone className="w-4 h-4 text-[#ef7f6d] group-hover:scale-110 transition-transform" />
                      ...
                      <div className="w-full space-y-8">
                        <button
                          ...
                          className="flex items-center gap-2 text-[10px] md:text-[11px] font-black uppercase tracking-widest text-neutral-400 hover:text-blush transition-colors mx-auto"
                        >
                  ...
                  <div className="mt-16 flex flex-col items-center">
                    <p className="terms-text text-[13px] text-neutral-400 text-center leading-[1.6] opacity-70">
                      ...
                      <Link href="/terms" className="text-blush font-black uppercase tracking-widest hover:underline underline-offset-4 decoration-2">Terms & Conditions</Link>
                    </p>
                  </div>
                </div>
              </div>
